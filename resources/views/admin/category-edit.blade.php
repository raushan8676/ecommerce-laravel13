            ...
            <form method="POST" action="{{ route('admin.category.update',['id' => $category->id]) }}" enctype="multipart/form-data" class="bg-white rounded-xl shadow-sm border border-gray-100 p-8 space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="text" id="name" name="name" value="{{ old('name', $category->name) }}" placeholder="e.g. Samsung"
                    ...
                    <div>
                        ...
                        <input type="text" id="slug" name="slug" value="{{ old('slug', $category->slug) }}" placeholder="samsung"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <select id="parent_id" name="parent_id" value="{{ old('parent_id', $category->parent_id) }}"
                            class="w-full border @error('parent_id') border-red-500 @else border-gray-300 @enderror px-4 py-2 rounded-lg outline-none focus:ring-1 focus:ring-primary">
                ...
                <div class="flex flex-col md:flex-row gap-8 items-start pt-4">
                    ...
                    <div class="w-full md:w-1/3">
                        ...
                        <div class="h-40 w-full border rounded-lg bg-white flex items-center justify-center p-4">
                            <img src="{{ asset('uploads/categories/thumbnails/' . $category->image) }}" class="max-h-full max-w-full object-contain" alt="{{ $category->name }}" onerror="this.src='https://placehold.co/80x80?text=Logo'">
                        </div>
                    </div>
                    @endif
                    <div class="w-full md:w-2/3">
                        ...
                        <div class="relative w-full h-40">
                            <label for="category-image" class="relative flex flex-col items-center justify-center w-full h-full border-2 @error('image') border-red-400 @else border-gray-300 @enderror border-dashed rounded-lg cursor-pointer bg-gray-50 hover:bg-gray-100 transition overflow-hidden">
                                
                                <div id="upload-content" class="text-center z-10">
                                    <i class="fa-solid fa-image text-3xl text-gray-300 mb-2"></i>
                                    <p class="text-sm text-gray-500">Upload new logo (PNG/JPG/WEBP)</p>
                                </div>
                ...
                <div class="flex items-center gap-2">
                    <input type="checkbox" id="status" name="status" value="1" {{ old('status', $category->status) == 1 ? 'checked' : '' }}
                ...
                <div class="flex justify-end gap-3 pt-4 border-t">
                    ...
                    <button type="submit"
                        class="px-6 py-2 bg-primary text-white rounded-lg hover:bg-blue-600 transition text-sm font-medium shadow-sm">Update Category</button>
                </div>
            </form>
        </div>
    </main>
