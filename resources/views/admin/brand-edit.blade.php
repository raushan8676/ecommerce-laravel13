            ...
            <form method="POST" action="{{ route('admin.brand.update',['id' => $brand->id]) }}" enctype="multipart/form-data" class="bg-white rounded-xl shadow-sm border border-gray-100 p-8 space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="text" id="name" name="name" value="{{ old('name', $brand->name) }}" placeholder="e.g. Samsung"
                    ...
                    <div>
                        ...
                        <input type="text" id="slug" name="slug" value="{{ old('slug', $brand->slug) }}" placeholder="samsung"
                ...
                <div class="flex flex-col md:flex-row gap-8 items-start pt-4">
                    ...
                    <div class="w-full md:w-1/3">
                        ...
                        <div class="h-40 w-full border rounded-lg bg-white flex items-center justify-center p-4">
                            <img src="{{ asset('uploads/brands/thumbnails/' . $brand->image) }}" class="max-h-full max-w-full object-contain" alt="{{ $brand->name }}" onerror="this.src='https://placehold.co/80x80?text=Logo'">
                        </div>
                    </div>
                    @endif
                    <div class="w-full md:w-2/3">
                        ...
                        <div class="relative w-full h-40">
                            <label for="brand-image" class="relative flex flex-col items-center justify-center w-full h-full border-2 @error('image') border-red-400 @else border-gray-300 @enderror border-dashed rounded-lg cursor-pointer bg-gray-50 hover:bg-gray-100 transition overflow-hidden">
                ...
                <div class="flex items-center gap-2">
                    <input type="checkbox" id="status" name="status" value="1" {{ old('status', $brand->status) == 1 ? 'checked' : '' }}
